...
function MultiCarousel2(props){
    ...
    const investmentClasses = () => {
        return(
            <Carousel key={props.data.map(id => id)} partialVisible={true} responsive={responsive} rtl={false} customLeftArrow={<div></div>} customRightArrow={<div></div>} className="">
            {
                props.data.map((option, id) => (
                    <div className={"Venture bluey p-3 rounded-4 mx-mc mt-1"} key={id} onClick={() => props.getWithdraws(option.name,option.total,option.investment_id, props.summ(option.data), option.handler)}>
                    <div className='row justify-content-center p-0' key={id}>
                        <div className=''><h5 className='bolder text-white pb-1'>{option.name}<h6>{option.handler}</h6></h5> </div>
                                    ...
                                    <div className = "d-flex flex-row flex m-0 justify-content-end" >< span className='bolder text-white'> { props.getCurrency(props.country) } </span>  
